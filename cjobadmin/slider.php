...
if($_SESSION['FLAG']=='1')
{
	...
	<html lang="en">
		<head>
			...
			<meta charset="utf-8" />
			<title>Slider - Manage-Slider</title>
						...
						<div class="page-content">
							...
							<div class="page-header">
								<h1>
									...
									<button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#create-item">Add Slider</button>
								</h1>
							</div><!-- /.page-header -->						
							...
							<div class="panel1 panel-primary">
								<div class="panel-heading">Show Slider</div>
								<div class="panel-body1 table-responsive">
									<table id="exam" class="table table-bordered">
										...
										{
											mysqli_query($db,"update cj_slider set status='0' where id='".$_REQUEST["status"]."'");
											header("location:slider.php");
											exit();
										}
										if(@$_REQUEST["change_statuss"]=='0' && isset($_REQUEST["status"]))
										{
											mysqli_query($db,"update cj_slider set status='1' where id='".$_REQUEST["status"]."'");
											header("location:slider.php");
											exit();
										}
										...
										$query = "SELECT * FROM `cj_slider` where 
										title LIKE '%".@$_REQUEST["search"]."%' OR 
										slider_desc LIKE '%".@$_REQUEST["search"]."%' OR
										url LIKE '%".@$_REQUEST["search"]."%' OR
										order_no LIKE '%".@$_REQUEST["search"]."%' OR
										url LIKE '%".@$_REQUEST["search"]."%' OR 
										modification_time LIKE '%".@$_REQUEST["search"]."%' LIMIT $start, $perpage";
										...
										while($row = mysqli_fetch_array($run_query))
										{
										 ...
										 ?>
											<tr>
												...
												if (!file_exists("../cjobadmin/images/sliderimage/".$image) || $image=='')
												{?>
													<td><img src="../image/dp/profile.png ?>" height="60" width="100"></td>

												<?php
												}else { ?>
												<td><img src="images/sliderimage/<?php echo "$image" ?>" height="60" width="100"></td>
												<?php	} ?>
													...
													<div class="modal-content">
													  ...
													  <div class="modal-body">
															...
															<div class="form-group">
																...
																<input type="text" name="title" id="title" class="form-control" data-error="Please Enter title" required="" value="<?php echo "$title"; ?>">
																<div class="help-block with-errors"></div>
															</div>

															<div class="form-group">
																<label class="control-label" for="name">Description:</label>
																<input type="name" name="desc" id="desc" class="form-control" data-error="Please enter name." required value="<?php echo "$slider_desc"; ?>"/>
																<div class="help-block with-errors"></div>
															</div>
															<div class="form-group">
																<label class="control-label" for="url">URL:</label>
																<input type="url" name="url" id="url" class="form-control" data-error="Please enter url." required value="<?php echo "$url"; ?>"/>
																<div class="help-block with-errors"></div>
															</div>
															
															
															<div class="form-group">
																<label class="control-label" for="order no">Order no:</label>
																<input type="number" name="order_no" id="order_no" class="form-control" data-error="Please enter order no." required value="<?php echo "$order_no"; ?>"/>
																<div class="help-block with-errors"></div>
															</div>
															<div class="form-group">
																<label class="control-label" for="title">Upload image:</label>
																<img style="padding: 5px;" id="img<?=$id;?>" src="images/sliderimage/<?php echo "$image" ?>" height="60" width="100" onError="this.src='../image/dp/profile.png'"/>
												...
												<td><a href="php/uploadslider.php?delete=<?php echo "$id"; ?>" class="btn btn-danger btn-xs" ><i class="fa fa-trash"></i> Delete</a></td>
								...
								<div class="modal-content">
								  ...
								  <div class="modal-body">
										<form data-toggle="validator" id="form" action="" method="POST" enctype="multipart/form-data">
											...
											<div class="form-group">
												<label class="control-label" for="name">Description:</label>
												<input type="name" name="desc" id="desc" class="form-control" data-error="Please enter name." required />
												<div class="help-block with-errors"></div>
											</div>
											<div class="form-group">
												<label class="control-label" for="url">URL:</label>
												<input type="url" name="url" id="url" class="form-control" data-error="Please enter url." required />
												<div class="help-block with-errors"></div>
											</div>
											...
											<div class="form-group">
												<label class="control-label" for="order no">Order no:</label>
												<input type="number" name="order_no" id="order_no" class="form-control" data-error="Please enter order no." required />
												<div class="help-block with-errors"></div>
											</div>
											<div class="form-group">
												<label class="control-label" for="title">Upload image:</label>
	...
	if(isset($_POST['update']))
	{
		...
		$name = $_POST['desc'];
		$email = $_POST['url'];    
		$startdate = $_POST['order_no'];
		$id = $_POST['id_val'];


		if($_FILES["image"]["name"]!='')
		{
			$img_nm1=mysqli_fetch_array(mysqli_query($db,"select image from cj_slider where id='$id'"));
			@unlink("images/sliderimage/".$img_nm1['image']);
			...
			{
				move_uploaded_file($_FILES["image"]["tmp_name"],"images/sliderimage/".$image);
			}	
		}
		else
		{
		...
		mysqli_query($db,"UPDATE cj_slider SET 
		title='$heading',
		slider_desc='$name',
		url='$email',
		order_no='$startdate',
